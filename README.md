...
The system is built from the view of a Library attendant. In this system, each reader has to deposit Ksh. 500 or above during registration. Readers and books can be registered and searched from the respective pages and will be automatically to respective pages as well. A book can be added to bag for checkout, where the attendant can search for a reader using their ID and check them out after entering the date of borrowing and expected return date. If the user's balance is below Ksh.500, they are expected to make a deposit to allow them to take a book from the library. If checkout is suucessful, the checkout record is stored in the returns file. The attendant can view all previous checkouts by a reader by searching their name or ID. 

Permissions to deactivate, or delete book and reader records are only via the admin portal. 
